const CONFIG: {BLOG_NAME: string, HUE: number, DO_DAYNIGHT_CYCLE:boolean, ABOUT: string, CONTACT: {method:string, value:string}[]} = {
    ...
    ABOUT: "This is a blog template, just a quick project in React for some styling practice, and to fill out my git. The blog's title, color scheme, and posts are pulled from .json files. I may go add an actual backend for posts later. The page is dynamically colored, currently it's set up to simulate a day/night cycle, changing colour throughout the day, depending on device time. Special thanks to all the library contributors that made this possible.", //description of blog
    CONTACT: [ //contact details
        {method: "phone", value: "123 456 789"},
        {method: "email", value: "example@email.com"},
        {method: "address", value: "1 Fake Lane, London"}
    ]
}
